...
func (m *ResponseMark) reqID() uint16 {
	return m.ReqID
}

func (m *ResponseMark) reqNum() uint16 {
	return m.ReqNum
}
...
// // 获取子包（大数据包时有效）
// func (m *multiPacket) nextPacket() (*singlePacket, error) {
// 	if len(m.pktIdxs) == 0 {
// 		return nil, io.EOF
// 	}

// 	idx := m.pktIdxs[0]
// 	m.pktIdxs = m.pktIdxs[1:]

// 	size, off := len(m.body), int(idx-1)*maxBodySize
// 	if off < 0 || off > size {
// 		return nil, fmt.Errorf("包序号溢出[当前请求：%d，总包个数：%d]", idx, m.head.pack.total)
// 	}

// 	// 创建子包
// 	var single singlePacket
// 	single.head = m.head
// 	single.head.pack.index = idx
// 	if (size - off) < maxBodySize {
// 		single.body = make([]byte, size-off)
// 	} else {
// 		single.body = make([]byte, maxBodySize)
// 	}
// 	single.head.attr.setBodySize(uint16(len(single.body)))

// 	copy(single.body, m.body[off:])
